...
int NeptuneProtocol::getClockPin() const
{
  return clock_pin;
}
...
void NeptuneProtocol::readMeter(reading *meterRead)
{
  ...
  if ((meterData[0] == 0) || (meterData[0] == -1) || (meterData[14] == 0xff))
  {  // Not sure about the oxFF but that was contained in the string in a error reading so catching it here... sigh.
